...
func main() {
    let (_, q) = read().asList(ofInt).asTuple()
    let w = read().asList(ofInt)
    
    let uf = UnionFind(weights: w)
    
    for _ in 0 ..< q {
        let (t, x, y) = read().asList(ofInt).asTuple()
        switch t {
        case 0:
            uf.unite(x, y)
        case 1:
            print(uf.weightOfGroup(of: x))
        default:
            break
        }
    }
...
class UnionFind {
    ...
    private var weights: [Int]    // このグループ内の要素の重さの合計（根の要素のもののみ有効）
    
    /// 要素数を指定して構築（各要素は別のグループに所属していることになる）
    /// - Parameter weights 各要素の重さ
    init(weights: [Int]) {
        parents = .init(repeating: -1, count: weights.count)
        ranks = .init(repeating: 0, count: weights.count)
    ...
    func unite(_ x: Int, _ y: Int) -> Bool {
        ...
        ranks[rx] = rank
        let weight = weights[ry]
        weights[rx] += weight
        return true
    }
    
    /// 指定した要素番号の要素を含むグループの重さを求めます
    /// - Parameter x: グループに含まれる要素
    /// - Returns: xを含むグループの合計の重さ
    func weightOfGroup(of x: Int) -> Int {
        return weights[root(of: x)]
    }
}
